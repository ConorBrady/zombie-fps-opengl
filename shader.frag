...
void main () {
	...
	for(int i = 0; i < 5; i ++){
		...
		float light_intensity = min(max(1.0/pow(distance_from_light_to_position/light_reach,2.0),0.0),1.0)*abs(sin(dot(position_to_light_dir_eye,normal_eye)*10+time*5));
	...
	float strength = 0;
	for(int i = 0; i < 5; i++) {
		if(light_position_world[i] != vec3(0)) {
			vec3 Ls = light_properties[i][0];
			vec3 Ld = light_properties[i][1];
			vec3 La = light_properties[i][2];
